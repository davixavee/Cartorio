(() => {
  ...
  function getVisibleInputs() {
    ...
    if (!tipo) return [];
    return qsa(`#${tipo} input, #${tipo} select`);
  }
  ...
  function onSave(e) {
    ...
    if (editId) {
      ...
      if (idx >= 0)
        registros[idx] = { id: editId, tipo: getActiveTipo(), dados };
      editId = null;
    } else {
  ...
  function init() {
    qsa(".tab").forEach((t) =>
      t.addEventListener("click", () => setActiveTab(t.dataset.tipo))
    );
    ...
    if (active && active.dataset && active.dataset.tipo)
      setActiveTab(active.dataset.tipo);
