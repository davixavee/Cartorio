...
export class CidadesEstados {
    ...
    this.getEstados()
      .then((response) => {
        ...
        estadosObj.forEach((estadoObj) => {
          ...
          estadoObj.options[estadoObj.options.length] = new Option(
            "Selecione um estado",
            ""
          );

          this.json.estados.forEach((estado) => {
            estadoObj.options[estadoObj.options.length] =
              estado.id == estadoAttr
                ? new Option(estado.estado, estado.id, null, true)
                : new Option(estado.estado, estado.id);
          });
  ...
  renderCidades(estado, grupoId) {
    let cidadeObj = document.querySelector(
      `[data-cidade][data-grupo-id='${grupoId}']`
    );
    ...
    cidadeObj.options[cidadeObj.options.length] = new Option(
        ...
        .then((response) => {
          response.cidades.forEach((cidade) => {
            if (cidadeAttr == "" && cidade.capital) {
              ...
              cidadeObj.options[cidadeObj.options.length] =
                cidade.cidade == cidadeAttr
                  ? new Option(cidade.cidade, cidade.cidade, null, true)
                  : new Option(cidade.cidade, cidade.cidade);
            }
          });
        })
        .catch((err) => console.log(err));
  ...
  changeEstados() {
    document.addEventListener("change", (event) => {
      if (event.target.matches("[data-estado]")) {
        ...
        let grupoIdAttr = estadoObj.getAttribute("data-grupo-id");
        let cidadeObj = document.querySelector(
          `[data-cidade][data-grupo-id='${grupoIdAttr}']`
        );
  ...
  changeCidades() {
    document.addEventListener("change", (event) => {
      if (event.target.matches("[data-cidade]")) {
        ...
        cidadeObj.setAttribute(
          "data-cidade",
          cidadeObj[cidadeObj.selectedIndex].text
        );
      }
    });
  }
